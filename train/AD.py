...
def parse_args():
    ...
    parser.add_argument('--ad-enable', action='store_true',
                        help='Enable ADSAL-style Applicability Domain (ρs & IA)')
    parser.add_argument('--ad-space', type=str, default='auto',
                        choices=['auto', 'Z', 'chemberta', 'padel', 'concat'],
                        help="Representation space for AD: Z (fusion), chemberta, padel, or concat (original+Z). 'auto'优先用concat，其次Z，再次chemberta")
    parser.add_argument('--ad-k', type=int, default=50, help='k-NN neighbors for AD')
    parser.add_argument('--ad-tau', type=float, default=0.1,
                        help='temperature for exp(cos/τ) weighting (ρs, IA)')
    parser.add_argument('--ad-weight', type=str, default='exp',
                        choices=['exp', 'relu'],
                        help="权重函数: 'exp' = exp(cos/τ); 'relu' = max(cos, 0)")
    parser.add_argument('--ad-pca-dim', type=int, default=0,
                        help='(可选) 在AD前PCA降维到该维数(0=不降维)')
    parser.add_argument('--ad-grid', type=str, default='0.0,0.1,0.2,0.3',
                        help='分位数网格, 逗号分隔。ρs去底分位、IA去顶分位组合扫描')
...
def _maybe_pca(X_tr, X_va, n_components):
    ...
    Xtr = pca.fit_transform(X_tr)
    Xva = pca.transform(X_va)
    return Xtr.astype(np.float32), Xva.astype(np.float32), pca


def _standardize_features(X_tr, X_va, standardize=True):
    """对特征进行标准化（可选）"""
    if not standardize:
        return X_tr.astype(np.float32), X_va.astype(np.float32), None

    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    X_tr_std = scaler.fit_transform(X_tr).astype(np.float32)
    X_va_std = scaler.transform(X_va).astype(np.float32)
    return X_tr_std, X_va_std, scaler

def compute_ad_scores(
    Z_tr, Y_tr, Z_va, Y_va, k=50, tau=0.1, weight_mode='exp'
):
    """
    参照 ADSAL：ρs = k近邻加权相似度密度；IA = k近邻加权Jaccard距离(多标签)
    - 相似度: cos = 1 - cosine_distance
    - 权重: exp(cos/τ) 或 relu=max(cos,0)
    """
    assert Z_tr.shape[1] == Z_va.shape[1]
    # kNN（cosine distance），返回最近邻及距离
    knn = NearestNeighbors(n_neighbors=min(k, len(Z_tr)), metric='cosine', n_jobs=-1)
    knn.fit(Z_tr)
    dists, idxs = knn.kneighbors(Z_va, return_distance=True)
    # cos 相似度
    cos = 1.0 - dists  # [N_va, k]
    if weight_mode == 'exp':
        w = np.exp(cos / float(tau))
    else:
        w = np.maximum(cos, 0.0)
    # ρs：加权相似度密度（做平均归一，和论文“越大越在域内”一致）
    rho = (w.sum(axis=1) / w.shape[1]).astype(np.float32)

    # IA：邻域加权多标签不一致度（Jaccard 距离 = 1 - Jaccard 相似）
    # y ∈ {0,1}^L；对每个验证样本，拿邻居的 Y_tr 做加权平均距离
    N_va, k_eff = idxs.shape
    ia = np.zeros(N_va, dtype=np.float32)
    for i in range(N_va):
        y = Y_va[i]  # [L]
        neigh = Y_tr[idxs[i]]  # [k, L]
        inter = (neigh & y).sum(axis=1).astype(np.float32)
        union = (neigh | y).sum(axis=1).astype(np.float32)
        # 处理 union=0（双方全0） → 定义距离为0（无不一致）
        jac_sim = np.divide(inter, np.maximum(union, 1e-8))
        jac_dist = 1.0 - jac_sim  # [k]
        wi = w[i]
        ia[i] = float((wi * jac_dist).sum() / (wi.sum() + 1e-8))
    return rho, ia, idxs

def sweep_ad_thresholds(y_true, probs, rho, ia, grid_q_rho=(0.0,0.1,0.2,0.3), grid_q_ia=(0.0,0.1,0.2,0.3), label_names=None):
    """
    扫描严苛度→性能/覆盖率曲线：
      - ρs 取 “去底部分位”阈值：只保留 ρs >= q_rho 分位值
      - IA 取 “去顶部分位”阈值：只保留 IA <= q_ia 分位值
    返回：每组(q_rho, q_ia)下的覆盖率与一套综合指标
    """
    rows = []
    rho = rho.reshape(-1); ia = ia.reshape(-1)
    N = len(rho)
    for qr in grid_q_rho:
        thr_r = np.quantile(rho, qr)
        keep_r = (rho >= thr_r)
        for qi in grid_q_ia:
            thr_i = np.quantile(ia, 1.0 - qi)
            keep_i = (ia <= thr_i)
            keep = keep_r & keep_i
            cov = float(keep.mean())
            if cov <= 0:
            ...
            yt = y_true[keep]
            pb = probs[keep]
            # 使用原有评估函数，阈值用默认调过的 per-label 阈值或 0.5（我们这里统一用 0.5 保持简洁）
            thr_vec = np.full(yt.shape[1], 0.5, dtype=np.float32)
            overall, _ = compute_comprehensive_metrics(yt, pb, thr_vec, label_names)
            rows.append({
                "q_rho": float(qr), "q_ia": float(qi),
                "coverage": cov,
                **overall
            })
...
def main():
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(kfold.split(X_placeholder, Y_all)):
        ...
        if feature_type == "both" and args.fusion == "attn":
            ...
            (Z_tr, Z_va, _Zte), (Xrw_tr, Xrw_va, _Xrwte), (A_tr, A_va, _Ate) = train_fusion_and_transform(
                ...
                X_va_chem, Xpd_va,   # 这里仅需生成到验证集，无需test
                d=args.fusion_dim, epochs=args.fusion_epochs, batch=args.fusion_batch,
                # 替换这一行的参数名
                lr=args.fusion_lr, dropout=args.fusion_dropout, patience=args.fusion_patience
            ...
            if args.fusion_useZonly:
                ...
                feature_names = [f"chemBERTa_{i}" for i in range(X_tr_chem.shape[1])] + padel_cols + \
        ...
        np.save(f"results/cv5/fold{fold_idx}_y_true.npy", Y_va)   # <== 新增：保存真值

        # === [AD] 计算ρs/IA并输出严苛度→性能/覆盖率曲线 ===
        if getattr(args, "ad_enable", False):
            # 选择AD表征空间：优先concat，其次Z，再次chemberta，最后padel
            chosen = None
            ad_space = getattr(args, "ad_space", "auto")

            # 如果明确指定了concat或者auto模式下优先使用concat
            if (ad_space == "concat" or (
                    ad_space == "auto" and feature_type == "both" and args.fusion == "attn" and not args.fusion_useZonly)):
                # 使用拼接特征（与主模型相同的特征空间）
                # 先标准化，再PCA（如果需要）
                Xtr_ad, Xva_ad, _ = _standardize_features(X_tr_cat, X_va_cat, standardize=True)
                Xtr_ad, Xva_ad, _ = _maybe_pca(Xtr_ad, Xva_ad, getattr(args, "ad_pca_dim", 0))
                chosen = "concat"
            elif ad_space == "Z" or (ad_space == "auto" and feature_type == "both" and args.fusion == "attn"):
                try:
                    Ztr, Zva = Z_tr, Z_va
                    chosen = "Z"
                    Xtr_ad, Xva_ad, _ = _maybe_pca(Ztr, Zva, getattr(args, "ad_pca_dim", 0))
                except NameError:
                    pass

            if chosen is None and (
                    ad_space == "chemberta" or (ad_space == "auto" and feature_type in ["chemberta", "both"])):
                Xtr_ad, Xva_ad, _ = _maybe_pca(X_tr_chem, X_va_chem, getattr(args, "ad_pca_dim", 0))
                chosen = "chemberta"

            if chosen is None and (ad_space == "padel" or (ad_space == "auto" and feature_type in ["padel", "both"])):
                Xtr_ad, Xva_ad, _ = _maybe_pca(Xpd_tr, Xpd_va, getattr(args, "ad_pca_dim", 0))
                chosen = "padel"

            if chosen is not None:
                rho, ia, _ = compute_ad_scores(
                    Xtr_ad, Y_tr, Xva_ad, Y_va,
                    k=getattr(args, "ad_k", 50),
                    tau=getattr(args, "ad_tau", 0.1),
                    weight_mode=getattr(args, "ad_weight", "exp")
                )
                np.savez(f"results/cv5/fold{fold_idx}_AD_{chosen}.npz", rho=rho, IA=ia)

                # 扫描严苛度网格（分位数），得到性能-覆盖率曲线
                grid_vals = tuple(float(x) for x in getattr(args, "ad_grid", "0.0,0.1,0.2,0.3").split(",") if x)
                df_curve = sweep_ad_thresholds(
                    y_true=Y_va, probs=Pva, rho=rho, ia=ia,
                    grid_q_rho=grid_vals, grid_q_ia=grid_vals,
                    label_names=label_cols
                )
                df_curve.to_csv(f"results/cv5/ad_curve_{chosen}_fold{fold_idx}.csv", index=False)
                print(f"[AD] fold{fold_idx}: space={chosen} | curve points={len(df_curve)}")
            else:
                print("[AD] No valid representation for AD in this fold; skipped.")
    ...
    # === (可选) 聚合AD曲线 across folds，得到均值±std ===
    if getattr(args, "ad_enable", False):
        # 支持自动空间（如每折可能不同）；你也可以固定只画 Z
        for space in ["concat", "Z", "chemberta", "padel"]:
            fold_csvs = []
            for fi in range(args.cv_folds):
                p = f"results/cv5/ad_curve_{space}_fold{fi}.csv"
                if os.path.exists(p):
                    fold_csvs.append(pd.read_csv(p))
            if not fold_csvs:
                continue
            df_all = pd.concat(fold_csvs, ignore_index=True)
            keys = ["q_rho", "q_ia", "coverage"]
            metric_cols = [c for c in df_all.columns if c not in keys]
            g = df_all.groupby(keys, as_index=False)[metric_cols].agg(['mean','std'])
            g.columns = ['_'.join(c).strip('_') for c in g.columns.to_flat_index()]
            outp = f"results/cv5/ad_curve_{space}_summary.csv"
            g.to_csv(outp, index=False)
            print(f"[AD] aggregated curve saved: {outp}")
